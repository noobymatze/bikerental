...
if [ ! -d MYSQL_WILDFLY_DIR ]; then
    ...
    sed -i "s/<resource-root path=\".*\"\/>/<resource-root path=\"${MYSQL_JAR_FILE}\"\/>/" $MYSQL_WILDFLY_DIR/module.xml
...
'/subsystem=datasources/data-source=BikerentalDS:add(connection-url=jdbc:mysql://localhost:3317/bikerental,driver-name=mysql,jndi-name=java:/datasources/BikerentalDS,use-java-context=true,user-name=wildfly,password=wildfly,enabled=true)'

sudo chown -R vagrant $WILDFLY_DIR

mv $WILDFLY_DIR /vagrant
